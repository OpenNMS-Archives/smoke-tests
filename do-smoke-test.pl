...
sub run_smoke_tests {
	...
	system($XVFB_RUN, '--wait=20', '--server-args=-screen 0 1920x1080x24', '--server-num=80', '--auto-servernum', '--listen-tcp', '../compile.pl', '-Dsurefire.rerunFailingTestsCount=5', '-Dfailsafe.rerunFailingTestsCount=5', '-Dorg.opennms.smoketest.logLevel=INFO', '-Dsmoke=true', '-t') == 0 or fail(1, "failed to run smoke tests: $!");
}
